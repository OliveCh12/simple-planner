...
const items: PlanItem[] = [
  ...
  item({ id: "overdue", kind: "task", parentId: "obj", start: "2026-08-01", end: "2026-08-03" }),
];

const octoberWeek = { start: new Date(2026, 9, 12), end: new Date(2026, 9, 19) };

describe("planningContext", () => {
  it("keeps year view at objectives only", () => {
    ...
    expect(context.objectives[0].events).toEqual([]);
    expect(context.prep).toEqual([]);
    expect(context.open).toEqual([]);
  });

  it("lists event prep in week view and overdue work still open", () => {
    ...
    expect(context.open.map((task) => task.id)).toEqual(["overdue"]);
  });
});
