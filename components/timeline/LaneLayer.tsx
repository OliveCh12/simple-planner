"use client";

import { ClusterBar } from "@/components/timeline/ClusterBar";
import { ItemBar } from "@/components/timeline/ItemBar";
import type { DragPreview } from "@/hooks/useTaskPointer";
import {
  ALL_DAY_LANE_PX,
  TIMED_LANE_PX,
  type LaneItem,
  type LaneStack,
} from "@/lib/lanes";
import { cn } from "@/lib/utils";
import type { PlanItem, TimeScale } from "@/types";

interface LaneLayerProps {
  stack: LaneStack;
  items: LaneItem[];
  itemsById: Map<string, PlanItem>;
  variant: "allDay" | "timed";
  scale: TimeScale;
  fromX: number;
  preview: DragPreview | null;
  highlightId?: string | null;
}

function laneHeight(variant: "allDay" | "timed", scale: TimeScale): number {
  if (variant === "allDay") return ALL_DAY_LANE_PX;
  return scale === "day" || scale === "hour" ? TIMED_LANE_PX.fine : TIMED_LANE_PX.coarse;
}

export function LaneLayer({
  stack,
  items,
  itemsById,
  variant,
  scale,
  fromX,
  preview,
  highlightId,
}: LaneLayerProps) {
  const height = laneHeight(variant, scale);
  const lanes = Math.max(stack.laneCount, 1);
  const pad = 6;

  return (
    <div
      data-lane-layer={variant}
      className={cn("relative shrink-0", variant === "allDay" && "border-b border-border/50")}
      style={{ minHeight: lanes * height + pad * 2 }}
    >
      {items.map((item) => {
        const top = pad + item.lane * height;
        if (item.kind === "cluster") {
          const planItems = item.memberIds
            .map((id) => itemsById.get(id))
            .filter((entry): entry is PlanItem => Boolean(entry));
          return (
            <ClusterBar
              key={item.id}
              item={item}
              tasks={planItems}
              top={top}
              height={height - 2}
              fromX={fromX}
            />
          );
        }
        const planItem = itemsById.get(item.itemId);
        if (!planItem) return null;
        return (
          <ItemBar
            key={item.id}
            item={item}
            planItem={planItem}
            top={top}
            height={height - 2}
            variant={variant}
            fromX={fromX}
            preview={preview}
            highlight={highlightId === planItem.id}
          />
        );
      })}
    </div>
  );
}
